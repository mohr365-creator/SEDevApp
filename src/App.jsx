  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard title="Validated" value={`${Math.round((stats.validated / stats.total) * 100) || 0}%`} subtitle={`${stats.validated}/${stats.total} reqs`} icon={<CheckCircle className="text-green-500" />} color="green" />
        <StatCard title="Verified" value={`${Math.round((stats.verified / stats.total) * 100) || 0}%`} subtitle={`${stats.verified}/${stats.total} reqs`} icon={<ShieldCheck className="text-indigo-500" />} color="indigo" />
        <StatCard title="Certified" value={`${Math.round((stats.certified / stats.total) * 100) || 0}%`} subtitle={`${stats.certified}/${stats.total} reqs`} icon={<Award className="text-amber-500" />} color="amber" />
        <StatCard title="Safety Critical" value={stats.safetyCritical} subtitle="DAL A / Safety Type" icon={<AlertTriangle className="text-red-500" />} color="red" />
      </div>
...
// ==========================================
// 3.7 CERTIFICATION VIEW
// ==========================================

const CertificationView = ({ requirements, onStatusChange, onLinkArtifact }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState('All');

  const certificationStats = useMemo(() => {
    const total = requirements.length;
    const certified = requirements.filter(r => r.certificationStatus === 'Certified').length;
    const pending = requirements.filter(r => r.certificationStatus === 'Pending Review').length;
    const inProgress = requirements.filter(r => r.certificationStatus === 'In Progress').length;
    const notStarted = requirements.filter(r => !r.certificationStatus || r.certificationStatus === 'Not Started').length;
    const findings = requirements.filter(r => r.certificationStatus === 'Finding Open').length;
    
    return { total, certified, pending, inProgress, notStarted, findings };
  }, [requirements]);
  ...
  const filteredReqs = requirements.filter(req => {
    ...
    const matchesFilter = filterStatus === 'All' || 
      (filterStatus === 'Certified' && req.certificationStatus === 'Certified') ||
      (filterStatus === 'Pending' && req.certificationStatus === 'Pending Review') ||
      (filterStatus === 'In Progress' && req.certificationStatus === 'In Progress') ||
      (filterStatus === 'Finding' && req.certificationStatus === 'Finding Open') ||
      (filterStatus === 'Not Started' && (!req.certificationStatus || req.certificationStatus === 'Not Started'));
    
    return matchesSearch && matchesFilter;
  });

  const getReadinessScore = () => {
    const validatedCount = requirements.filter(r => r.validationStatus === 'Validated').length;
    const verifiedCount = requirements.filter(r => r.verificationStatus === 'Verified').length;
    const certifiedCount = requirements.filter(r => r.certificationStatus === 'Certified').length;
    
    if (requirements.length === 0) return 0;
    return Math.round(((validatedCount + verifiedCount + certifiedCount) / (requirements.length * 3)) * 100);
  };
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-2xl font-bold text-slate-900">7. Certification</h2>
        </div>
        <p className="text-slate-600 italic">"Is it approved for use?"</p>
        <p className="text-sm text-slate-500 mt-2">
          Track certification status, regulatory approvals, findings, and compliance documentation for airworthiness certification.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 border-l-4 border-l-amber-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-slate-500">Certification Readiness</p>
              <p className="text-2xl font-bold text-amber-600">{getReadinessScore()}%</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-green-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{certificationStats.certified}/{certificationStats.total}</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-blue-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">{certificationStats.pending}</p>
            </div>
        ...
        <Card className="p-4 border-l-4 border-l-red-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">{certificationStats.findings}</p>
            </div>
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          <div>
            <h3 className="text-md font-semibold text-slate-900">Certification Activities</h3>
            <p className="text-xs text-slate-500">Track regulatory compliance and certification status</p>
          </div>
          <div className="flex gap-2">
            {['All', 'Certified', 'Pending', 'In Progress', 'Finding', 'Not Started'].map(status => (
              <button
                key={status}
        ...
        <input 
          type="text" 
          className="pl-3 pr-3 py-2 border border-slate-300 rounded-lg bg-slate-50 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500" 
          placeholder="Search requirements..." 
          value={searchTerm} 
          onChange={(e) => setSearchTerm(e.target.value)} 
        />
      ...
      <div className="space-y-4">
        {filteredReqs.map(req => (
          <Card key={req.id} className="p-0 overflow-hidden hover:shadow-md transition-shadow border-l-4 border-l-amber-500">
            <div className="p-5">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-sm font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded">{req.id}</span>
                  <h4 className="text-lg font-semibold text-slate-900">{req.title}</h4>
                  <Badge color={req.type === 'Safety' ? 'red' : 'blue'}>{req.type}</Badge>
                  <Badge color={req.criticality === 'DAL A' ? 'red' : req.criticality === 'DAL B' ? 'orange' : 'yellow'}>{req.criticality}</Badge>
                </div>
                <div className="relative">
                  <select
                    value={req.certificationStatus || 'Not Started'}
                    onChange={(e) => onStatusChange(req.id, 'certificationStatus', e.target.value)}
                    className={`appearance-none pl-3 pr-8 py-1.5 rounded-lg text-sm font-medium border cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-500 ${
                      req.certificationStatus === 'Certified' ? 'bg-green-100 text-green-800 border-green-200' :
                      req.certificationStatus === 'Pending Review' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                      req.certificationStatus === 'In Progress' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                      req.certificationStatus === 'Finding Open' ? 'bg-red-100 text-red-800 border-red-200' :
                      'bg-slate-100 text-slate-800 border-slate-200'
                    }`}
                  >
                    ...
                    <option value="Finding Open">Finding Open</option>
              ...
              <div className="flex gap-4 mb-4">
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
                  req.validationStatus === 'Validated' ? 'bg-green-50 text-green-700' : 'bg-slate-50 text-slate-500'
                }`}>
                  <CheckCircle size={14} />
                  <span>Validation: {req.validationStatus}</span>
                </div>
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${
                  req.verificationStatus === 'Verified' ? 'bg-indigo-50 text-indigo-700' : 'bg-slate-50 text-slate-500'
                }`}>
                  <ShieldCheck size={14} />
                  <span>Verification: {req.verificationStatus}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  <label className="text-xs font-semibold text-slate-500 block mb-1">Certification Basis</label>
                  <div className="relative">
                    <select
                      value={req.certificationBasis || 'Not Specified'}
                      onChange={(e) => onStatusChange(req.id, 'certificationBasis', e.target.value)}
                      className="w-full appearance-none pl-2 pr-6 py-1.5 rounded text-sm font-medium border border-slate-300 bg-white cursor-pointer focus:outline-none focus:ring-1 focus:ring-amber-500"
                    >
                      <option value="Not Specified">Not Specified</option>
                      <option value="14 CFR Part 25">14 CFR Part 25</option>
                      <option value="14 CFR Part 23">14 CFR Part 23</option>
                      <option value="CS-25">EASA CS-25</option>
                      <option value="CS-23">EASA CS-23</option>
                      <option value="DO-178C">DO-178C</option>
                      <option value="DO-254">DO-254</option>
                      <option value="ARP4754A">ARP4754A</option>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  <label className="text-xs font-semibold text-slate-500 block mb-1">Approval Authority</label>
                  <div className="relative">
                    <select
                      value={req.approvalAuthority || 'Not Specified'}
                      onChange={(e) => onStatusChange(req.id, 'approvalAuthority', e.target.value)}
                      className="w-full appearance-none pl-2 pr-6 py-1.5 rounded text-sm font-medium border border-slate-300 bg-white cursor-pointer focus:outline-none focus:ring-1 focus:ring-amber-500"
                    >
                      <option value="Not Specified">Not Specified</option>
                      <option value="FAA">FAA</option>
                      <option value="EASA">EASA</option>
                      <option value="FAA/EASA">FAA/EASA Joint</option>
                      <option value="DER">DER Authorized</option>
                      <option value="ODA">ODA Unit</option>
                      <option value="AR">AR (EASA)</option>
                    </select>
                    <ChevronDown size={12} className="absolute right-2 top-2 pointer-events-none text-slate-500" />
                  </div>
                </div>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  <label className="text-xs font-semibold text-slate-500 block mb-1">Compliance Method</label>
                  <div className="relative">
                    <select
                      value={req.complianceMethod || 'Not Specified'}
                      onChange={(e) => onStatusChange(req.id, 'complianceMethod', e.target.value)}
                      className="w-full appearance-none pl-2 pr-6 py-1.5 rounded text-sm font-medium border border-slate-300 bg-white cursor-pointer focus:outline-none focus:ring-1 focus:ring-amber-500"
                    >
                      <option value="Not Specified">Not Specified</option>
                      <option value="MC 1">MC 1 - Compliance Statement</option>
                      <option value="MC 2">MC 2 - Analysis/Calculation</option>
                      <option value="MC 3">MC 3 - Safety Assessment</option>
                      <option value="MC 4">MC 4 - Lab Test</option>
                      <option value="MC 5">MC 5 - Ground Test</option>
                      <option value="MC 6">MC 6 - Flight Test</option>
                      <option value="MC 7">MC 7 - Inspection</option>
                      <option value="MC 8">MC 8 - Simulation</option>
                      <option value="Equivalent Safety">Equivalent Safety</option>
                    </select>
                    <ChevronDown size={12} className="absolute right-2 top-2 pointer-events-none text-slate-500" />
                  </div>
                </div>
              ...
              <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-amber-800">Certification Documentation</span>
                  <button 
                    onClick={() => onLinkArtifact(req.id, 'certification')} 
                    className="inline-flex items-center gap-2 px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-medium rounded-lg transition-colors"
                  >
                    <Paperclip size={14} />
                    {req.certificationArtifact ? 'Update Document' : 'Link Document'}
                  </button>
                </div>
                {req.certificationArtifact ? (
                  <div className="flex items-center gap-2 text-sm text-amber-700 bg-white p-2 rounded border border-amber-200">
                    <FileCheck size={16} className="text-amber-600" />
                    <span className="font-mono text-xs">{req.certificationArtifact}</span>
                  </div>
                ) : (
                  <p className="text-xs text-amber-600 italic">No certification documentation linked</p>
                )}
              </div>

              {/* Finding Notes (if status is Finding Open) */}
              {req.certificationStatus === 'Finding Open' && (
                <div className="mt-3 bg-red-50 p-4 rounded-lg border border-red-200">
                  <div className="flex items-center gap-2 mb-2">
                    <AlertTriangle size={16} className="text-red-600" />
                    <span className="text-sm font-semibold text-red-800">Open Finding</span>
                  </div>
                  <textarea
                    value={req.findingNotes || ''}
                    onChange={(e) => onStatusChange(req.id, 'findingNotes', e.target.value)}
                    placeholder="Document the finding details and required corrective actions..."
                    className="w-full p-2 text-sm border border-red-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-red-500"
                    rows={2}
                  />
                </div>
              )}
      ...
      {filteredReqs.length === 0 && (
        <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-slate-500 mt-3">No requirements match your filter criteria</p>
        </div>
      )}
    </div>
  );
...
function App() {
  ...
  const stats = useMemo(() => ({
    ...
    certified: requirements.filter(r => r.certificationStatus === 'Certified').length,
  ...
  const handleLinkArtifact = (id, type) => {
    ...
    if (artifactName) {
      const field = type === 'validation' ? 'validationArtifact' : 
                    type === 'verification' ? 'verificationArtifact' : 
  ...
  return (
    <div className="min-h-screen bg-slate-50">
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-slate-200 py-2 z-50">
                    ...
                    <button
                      onClick={() => { setActiveView('certification'); setMenuOpen(false); }}
                      className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 hover:bg-slate-50 transition-colors ${activeView === 'certification' ? 'bg-slate-100 text-slate-900 font-semibold' : 'text-slate-700'}`}
                    >
                      <Award size={16} className="text-amber-500" />
                      <span>7. Certification</span>
                    </button>
                  </div>
                )}
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeView === 'certification' && <CertificationView requirements={requirements} onStatusChange={handleStatusChange} onLinkArtifact={handleLinkArtifact} />}
      </main>
